...
xplr.config.modes.builtin.default.key_bindings.on_key["ctrl-l"] =
  xplr.config.modes.builtin.default.key_bindings.on_key["ctrl-r"]
...
xplr.config.modes.builtin.default.key_bindings.on_key['ctrl-]'] =
  ...
  key_bindings = {
    on_key = {
      ...
      ['y'] = {
        ...
        messages = {
          { BashExecSilently = 'clipfile < "${XPLR_PIPE_RESULT_OUT:?}"' },
          'PopMode',
        },
      },
      ['Y'] = {
        help = 'copy path(s) to clipboard',
        messages = {
      ...
      ['d'] = {
        help = 'dragon-drop',
        messages = {
          { BashExecSilently = 'dragon-drop -I -s 64 < "${XPLR_PIPE_RESULT_OUT:?}"' },
          'PopMode',
        },
      },
      ['D'] = {
        help = 'dragon-drop all',
        messages = {
          {
            BashExecSilently = [===[
              all='-a'
              if [ 10 -lt "$(wc -l < "${XPLR_PIPE_RESULT_OUT:?}")" ]
              then
                all='-A'
              fi
              dragon-drop "$all" -I -s 64 < "${XPLR_PIPE_RESULT_OUT:?}"
            ]===],
          },
          'PopMode',
        },
...
xplr.config.modes.builtin.action = {
  ...
  key_bindings = {
    on_key = {
      ...
      ['Y'] = {
        help = 'copy parent dir path to clipboard',
        messages = {
          { BashExecSilently = 'wl-copy "${PWD:?}"' },
          'PopMode',
        },
      },
      ['d'] = {
        help = 'dragon-drop here',
        messages = {
          { BashExecSilently = 'dragon-drop -t -k -s 64 | xargs -rl curl -sLO' },
          'ExplorePwdAsync',
          'PopMode',
        },
      },
      ['p'] = {
        ...
        messages = {
          {
            BashExecSilently = [===[
              result="$(pastefile 2>&1 >/dev/null)"
              if [ -n "$result" ]
              then
                echo "LogError: '$result'" >> "${XPLR_PIPE_MSG_IN:?}"
              fi
            ]===],
          },
          'ClearScreen',
...
xplr.config.modes.builtin.filter.key_bindings.on_key['ctrl-l'] =
  xplr.config.modes.builtin.filter.key_bindings.on_key['ctrl-r']
...
xplr.config.modes.builtin.sort.key_bindings.on_key['ctrl-l'] =
  xplr.config.modes.builtin.sort.key_bindings.on_key['ctrl-r']
          ...
          messages = {
            {
              BashExecSilently = [===[
                TARGET="${XPLR_INPUT_BUFFER:?}"
                if [ ! -d "${TARGET:?}" ]; then
                  echo "LogError: 'dir $TARGET does not exists'" >> "${XPLR_PIPE_MSG_IN:?}"
                else
                  echo "ChangeDirectory: '${TARGET:?}'" >> "${XPLR_PIPE_MSG_IN:?}"
                fi
              ]===],
            },
            'PopMode',
...
-- Plugins
local xpm_home = os.getenv("XDG_DATA_HOME") .. '/xplr/xpm.xplr'
...
package.path = package.path
    ...
    .. xpm_home
    .. '/?/init.lua;'
os.execute(string.format('ls %s &>/dev/null || git clone %s %s', xpm_home, xpm_url, xpm_home))
